...
// (can be defined here or inside the class)
Vector3D
operator*(const Vector3D &v1, double d)
{
    return Vector3D(v1.getX() * d, v1.getY() * d, v1.getZ() * d);
}

Vector3D
operator*(double d, const Vector3D &v1)
{
    return Vector3D(v1.getX() * d, v1.getY() * d, v1.getZ() * d);
}
...
bool comp(const Vector3D &v1, const Vector3D &v2)
{
...
int main()
{
    ...
    std::vector<Vector3D> vecOfVec{vec0, vec1, vec2, vec4};
    std::sort(vecOfVec.begin(), vecOfVec.end()); // std::less()
    std::sort(vecOfVec.begin(), vecOfVec.end(), std::greater());
    std::sort(vecOfVec.begin(), vecOfVec.end(), comp);

    for (const Vector3D &v : vecOfVec)
        std::cout << v;
